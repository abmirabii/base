    ...
    Evm: ConfigureEvm,
{
    ...
    ///   from [`Self::validate_eip8130_structural`] before the inner Eth checks; verifier
    ...
    /// abstraction) transactions without requiring verifier dispatch or account
...
mod tests {
    ...
    #[test]
    fn accepts_eip8130_once_cobalt_active() {
        // Sanity-check the post-Cobalt side of the gate: the same transaction that is
        // rejected pre-Cobalt is admitted once the upgrade is active.
        let validator = build_test_validator();
        let signed = sign_eoa_eip8130(minimal_valid_eoa_tx());
        assert!(
            validator.validate_eip8130_structural(TransactionOrigin::External, &signed).is_ok()
